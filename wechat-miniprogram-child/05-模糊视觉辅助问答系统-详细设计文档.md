...
=======
    ...
    APIRouter *-- MediaRouter
    APIRouter *-- QARouter
    APIRouter *-- MemoryRouter
    APIRouter *-- AlertRouter
    APIRouter *-- ReminderRouter
>>>>>>> Stashed changes
```
...
=======
    ...
    get_db --> DB
    get_current_user --> AnyRole
    require_role --> ElderUser
    require_role --> ChildRole
    require_role --> AdminRole
>>>>>>> Stashed changes
    ElderUser --|> AnyRole
    ChildRole --|> AnyRole
    AdminRole --|> AnyRole
...
=======
详细描述：位于`app/api/v1/health.py`，提供系统健康检查接口，无需鉴权即可访问。
`GET /health`返回服务名称、运行环境和开发模式标志。
`GET /info`返回API版本、前缀和文档地址（Swagger/Redoc）。

##### 3.2 AuthRouter

详细描述：位于`app/api/v1/auth.py`，是系统身份认证的核心模块，共暴露八个接口端点。
`POST /auth/wx-login`接收WxLoginIn（code、user_type），DEV模式支持mock openid。流程：wx.code2session获取openid→查询或创建wx_account→签发access_token(30分钟)和refresh_token(7天)→返回TokenOut。
`POST /auth/refresh`解码refresh_token后重新签发access_token实现静默续期。
`POST /auth/logout`写入audit_log审计记录。
`POST /auth/admin/login`bcrypt哈希比对后签发admin类型JWT。
`POST /auth/bind-phone`要求ChildRole身份，更新child_user表phone字段。
`POST /auth/subscribe/grant`记录微信订阅消息授权结果到subscribe_grant表。
`GET /auth/dev/create-admin`仅在DEV模式可用，创建默认管理员admin/admin。
`POST /auth/heartbeat`更新user表last_heartbeat_at和online_status，检测离线恢复时设置was_offline标志，异常时通过`_create_network_alert()`创建预警并推送。

##### 3.3 MediaRouter

详细描述：位于`app/api/v1/media.py`，处理图片和语音上传，核心理念"上传即处理"。
`POST /media/upload/image`校验扩展名和大小（10MB），UUID保存原始文件后自动执行Gamma亮度增强（brightness=1.3），返回原图和增强图URL。
`POST /media/image/enhance`手动重新执行Gamma校正。
`POST /media/upload/voice`校验扩展名和大小（25MB），UUID保存后自动执行Whisper语音转文本，返回音频URL和asr_text。

##### 3.4 QARouter

详细描述：位于`app/api/v1/qa.py`，系统核心业务模块，实现完整的多模态智能问答处理管线。
`POST /qa/ask`接收QAAskIn（input_type/text/media_url/session_id），执行参数校验→上下文组装→Prompt获取→大模型调用→医疗合规拦截→预警检测→数据库落库→订阅消息推送八步管线，返回QAAskOut。
`WS /qa/stream/{task_id}`通过WebSocket实现流式问答，五种action帧：listen/speak/think/alert_signal/done，文本以chunk帧逐片推送。
`GET /qa/history`支持input_type/keyword/start_date/end_date多维筛选分页查询。
`GET /qa/history/{msg_id}`返回单条消息完整详情。

##### 3.5 MemoryRouter

详细描述：位于`app/api/v1/memory.py`，管理长期记忆生命周期和RAG语义检索。
`GET /memory`按user_id/memory_type/source/importance/keyword多维筛选分页列表。
`POST /memory`接收MemoryIn，双写MySQL+Chroma向量库。
`GET /memory/{memory_id}`单条记忆详情。
`PUT /memory/{memory_id}`编辑记忆，内容变更时重新向量化更新Chroma。
`DELETE /memory/{memory_id}`软删除，向量物理清理由定时任务异步执行。
`PATCH /memory/{memory_id}/importance`调整重要度1-5。
`POST /memory/search`语义检索：embed→Chroma余弦相似度→MySQL回查→按importance排序，Chroma不可用降级为MySQL LIKE搜索。

##### 3.6 AlertRouter

详细描述：位于`app/api/v1/alert.py`，处理老人端主动紧急呼叫。
`POST /alert/emergency/call`创建AlertEvent(alert_type=紧急呼叫, alert_level=紧急, trigger_source=manual)，遍历绑定子女推送alert.urgent订阅消息，写入communication_log，返回受通知子女数。

##### 3.7 ReminderRouter

详细描述：位于`app/api/v1/reminder.py`，老人端用药提醒只读查询。
`GET /reminder/medication`以user_id和active=1查询medication_reminder表，按remind_time升序返回。
>>>>>>> Stashed changes
...
=======
服务层封装核心业务逻辑，位于API控制层之下、数据访问层之上。
>>>>>>> Stashed changes
...
=======
    ...
    QARouter --> LLMClient
    MediaRouter --> ImageProcessor
    MediaRouter --> SpeechRecognition
    MemoryRouter --> EmbeddingService
    QARouter --> EmbeddingService
>>>>>>> Stashed changes
```
...
=======
详细描述：位于`app/services/llm_client.py`，封装大模型API调用，支持OpenAI兼容接口。模块级常量MEDICAL_KEYWORDS和ALERT_KEYWORDS定义拦截和预警规则库。
`call_llm(prompt, image_url, system_prompt, stream)`根据llm_api_key是否有值路由到_mock_call或_real_call。
`_mock_call()`DEV模式下生成预设话术，模拟延迟，命中涉医关键词追加免责声明，stream时逐字符yield。
`_real_call()`通过httpx.AsyncClient发送POST，构造OpenAI vision格式消息体，流式解析SSE事件流。
`check_medical_intercept(text)`返回(是否命中, 命中关键词列表)。
`check_alert_signal(text)`返回预警信号字典或None。
`append_disclaimer(answer)`追加免责声明。

##### 2.2 ImageProcessor

详细描述：位于`app/services/image_processor.py`，基于OpenCV+Pillow。ALLOWED_IMAGE_EXTS(jpg/jpeg/png/bmp/webp)，MAX_IMAGE_SIZE=10MB。
`enhance_image(src, dst, brightness)`核心Gamma校正：output=255*(input/255)^(1/brightness)，暗部提得多亮部提得少。

##### 2.3 SpeechRecognition

详细描述：位于`app/services/speech_recognition.py`，基于faster-whisper。MODEL_SIZE=small(500MB)，DEVICE=cpu，COMPUTE_TYPE=int8。
`_get_model()`懒加载单例，首次下载模型。
`transcribe(audio_path)`调用model.transcribe(language=zh, beam_size=5, vad_filter=True)。

##### 2.4 EmbeddingService

详细描述：位于`app/services/embedding.py`，文本转向量384维。优先all-MiniLM-L6-v2，降级_MockEmbedding(SHA256哈希种子+L2归一化)。
`embed(text)`单文本转向量。`embed_batch(texts)`批量转向量。`is_mock_embedding()`判断是否降级。
>>>>>>> Stashed changes
...
=======
    WxAccount --> User
    WxAccount --> ChildUser
    User --> UserChildRelation
    ChildUser --> UserChildRelation
>>>>>>> Stashed changes
```
...
=======
    ...
    ChromaClient --> SimpleVectorStore
    SimpleVectorStore *-- SimpleCollection
>>>>>>> Stashed changes
```
...
=======
老人端小程序（"喵喵助手"）采用微信原生小程序框架，适老化极简交互设计。
>>>>>>> Stashed changes
...
=======
详细描述：位于`pages/index/index`。`onShow()`时调用auth.restoreToken()尝试恢复登录态，有效token则redirectTo首页。`onLogin()`执行微信一键登录：wx.login→auth.login→持久化token→redirectTo首页。

##### 2.3 HomePage首页仪表盘

详细描述：位于`pages/home/home`。`onShow()`时校验token有效性，启动30秒heartbeat和5分钟reminder轮询。`fetchReminders()`对比缓存发现新增提醒时调用speech.speak()语音播报。`onHide()`和`onUnload()`清理定时器资源。四个导航方法跳转至chat/reminder/emergency/settings。

##### 2.4 ChatPage多模态智能聊天页

详细描述：位于`pages/chat/chat`，核心交互页面。四层结构：全屏camera后置摄像头取景→半透明深色遮罩→对话气泡scroll-view→底部操作栏。recordState(idle/recording/uploading/thinking)控制交互流程，catState(listen/think/speak)控制小猫动画。`onMicStart()`开始录音+拍首帧+midFrameTimer拍中帧。`onMicEnd()`停止录音+拍尾帧→upload上传→sendToQA()发送问答→speech.speak()语音播报。`sendToQA()`构造user消息→POST /qa/ask→解析回答→追加assistant消息→TTS播报→更新sessionId。

##### 2.5 ChatBubble对话气泡组件

详细描述：位于`components/chat-bubble`，唯一自定义组件。接收八个properties：role/type/content/status/catState/riskTags/intercepted/messageId。observers监听catState映射橘猫图片，监听content.images拼接完整URL。根据role渲染左对齐(assistant+猫头像)或右对齐(user)布局。status=sending显示loading动画，status=error显示重试按钮。intercepted=true显示免责声明。

##### 2.6 Request网络请求工具

详细描述：位于`utils/request.js`。`request(options)`自动从app.globalData获取token注入Authorization头，统一处理code==0展开data、code!=0展示Toast、HTTP 401跳转登录、网络异常提示。`getFullUrl(path)`拼接BASE_URL。

##### 2.7 Upload文件上传工具

详细描述：位于`utils/upload.js`。`uploadImage()`上传至/api/v1/media/upload/image，`uploadVoice()`上传至/api/v1/media/upload/voice，超时60秒。

##### 2.8 Auth鉴权工具

详细描述：位于`utils/auth.js`。`login()`串联wx.login→wx.request登录→持久化token到Storage和globalData。`restoreToken()`从Storage恢复token。`startHeartbeat()`启动30秒间隔的POST /auth/heartbeat。

##### 2.9 Speech语音播报工具

详细描述：位于`utils/speech.js`，基于WechatSI插件(wx069ba97219f66d99)。`speak(text)`检查voiceEnabled开关→plugin.textToSpeech合成mp3→wx.createInnerAudioContext播放。`stopSpeak()`销毁播放实例。`replayLast()`重播上次文本。

##### 2.10 Storage本地存储工具

详细描述：位于`utils/storage.js`。封装wx Storage API，所有key自动添加"gimi_"前缀。
>>>>>>> Stashed changes
...
=======
子女端小程序（"孝心守护"）采用Tab Bar三页主结构+三个分包按需加载的架构。
>>>>>>> Stashed changes
...
=======
    ...
    IndexPage --> ApiEndpoints
    AlertPage --> ApiEndpoints
>>>>>>> Stashed changes
```
...
=======
详细描述：位于`app.js`。`globalData`维护token/refreshToken/userId/nickname/userType/sessionId/boundElderlyId/boundElderlyName。onLaunch时从Storage恢复，onShow时启动30秒心跳，onHide时停止。

##### 2.2 ApiClient HTTP请求层

详细描述：位于`utils/api.js`。`request(options)`从Storage读取token注入Authorization头，超时15秒。HTTP 401时自动token刷新：refreshQueue队列+isRefreshing标志防并发→refreshToken()获取新token→重试请求。刷新失败调用clearLogin()。提供get/post/put/patch/del快捷方法。

##### 2.3 ApiEndpoints接口封装层

详细描述：位于`utils/api.js`中的api对象，封装15个后端API调用：login/heartbeat/bindElderly/unbindElderly/getBindedUsers/getDashboard/getMessages/getSettings/updateSettings/getSettingChanges/getAlerts/handleAlert/getMemory/createMemory/deleteMemory。

##### 2.4 IndexPage首页仪表盘

详细描述：位于`pages/index/index`，Tab Bar首个标签页。检查绑定状态：未绑定显示引导卡片，已绑定展示功能导航网格。

##### 2.5 AlertPage预警中心

详细描述：位于`pages/alert/alert`，Tab Bar第二个标签页。展示绑定老人的预警事件列表，支持下拉刷新。

##### 2.6 ProfilePage个人中心

详细描述：位于`pages/profile/profile`，Tab Bar第三个标签页。展示用户昵称和绑定老人姓名。`logout()`清空Storage和globalData后重启应用。

##### 2.7 分包页面

详细描述：HealthPage/MedicationPage/TracePage位于subpkg-health分包；MessagePage/GreetingPage/HistoryPage位于subpkg-care分包；BindPage/SettingsPage/KnowledgePage位于subpkg-manage分包。各页面当前为骨架占位——onShow检查绑定状态，UI布局和导航已定义，数据获取待后端接口就绪对接。
>>>>>>> Stashed changes
...
=======
定时任务模块使用asyncio原生协程，在FastAPI的lifespan生命周期中启动和关闭。
>>>>>>> Stashed changes
...
=======
    ...
    class AsyncSessionLocal
    class ChromaCollection
    FastAPI_Lifespan --> MonitorLoop
    MonitorLoop --> AsyncSessionLocal
    MonitorLoop --> ChromaCollection
>>>>>>> Stashed changes
```
...
=======
详细描述：`scan_offline_users()`每60秒执行。查询online_status='online'且last_heartbeat_at超过180秒的user记录→更新为offline→创建AlertEvent(alert_type=网络断开, alert_level=警告, trigger_source=rule)→去重检查→推送alert.warn订阅消息给绑定子女。

`clean_deleted_vectors()`每3600秒执行。查询is_deleted=1且vector_id非空的LongTermMemory(limit 500)→调用Chroma collection.delete()物理删除向量→vector_id置NULL。异步清理避免影响API响应时间。

`run_loop()`通过while True+asyncio.sleep()调度。在lifespan中通过create_task启动，关闭时task.cancel()优雅退出。
>>>>>>> Stashed changes
