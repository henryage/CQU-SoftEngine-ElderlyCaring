...
### 3.2 绑定老人 `POST /api/v1/child/bind`（验证码绑定）

**调用时机**：首次使用或添加新老人。

**前置流程**：
```
老人端 POST /auth/generate-bind-code → 拿到 "385721"
老人告知子女这 6 位数字
子女输入验证码 + 关系 → 绑定
```
...
```json
{"code": "385721", "relation": "母亲"}
```

| 参数 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| code | string | 是 | 老人端生成的 6 位验证码，5 分钟有效 |
| relation | string | 否 | 关系，默认"子女" |

**响应**

```json
{"code": 0, "msg": "绑定成功", "data": {"user_id": 6, "nickname": "妈妈", "relation": "母亲"}}
```

**后端逻辑**：查 `user` 表 `bind_code` 匹配 + 未过期 → 写入 `user_child_relation` → 清空验证码。验证码无效/过期返回 404，已绑定返回 409。
...
2. 老人端 POST /auth/generate-bind-code → 告知子女验证码 "385721"
3. POST /child/bind {code: "385721", relation: "母亲"}
4. GET /child/binded-users → 展示老人列表
5. 点击"母亲" → GET /child/dashboard/6 → 展示看板
```
...
```
1. 老人打开小程序 → 点击"添加子女"
2. POST /auth/generate-bind-code → 拿到 6 位验证码 "385721"
3. 老人在页面看到验证码，告知子女
4. 子女在小程序输入 "385721" + "母亲" → POST /child/bind
```
...
绑定      POST /child/bind              {code, relation}
